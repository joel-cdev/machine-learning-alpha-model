...
if run_model:
    with st.spinner("Training model and running backtest..."):
        ...
        if model_choice == "Random Forest":
            ...
            X_lstm, y_lstm, _ = create_lstm_sequences(
                df,
                FEATURE_COLUMNS,
                sequence_length=20,
            )
            ...
            predictions, metrics = evaluate_lstm_model(
                model,
                X_lstm_test,
                y_lstm_test,
            )

            plot_y_test = df["Target"].iloc[-len(y_lstm_test):]

            backtest = create_backtest_results(
                y_test=plot_y_test,
    ...
    ax2.scatter(plot_y_test, predictions, alpha=0.5)
